...
def batch_predict_varying_T(latent_dim, covar_module0, covar_module1, likelihoods, prediction_x, 
    ...
    K0xz_iH_K0zx_iB_mu_st = torch.matmul(K0xz, torch.linalg.solve(H, torch.matmul(K0zx, iB_mu))) #amélioré manon
    ...
    K0Xz_iK0zz_K0zx_mu_tilde = torch.matmul(K0Xz, torch.linalg.solve(K0zz, torch.matmul(K0zx, mu_tilde))) #amélioré manon 
...
def batch_predict(latent_dim, covar_module0, covar_module1, likelihoods, prediction_x, test_x, mu, 
    ...
    K0xz_iH_K0zx_iB_mu_st = torch.matmul(K0xz, torch.linalg.solve(H, torch.matmul(K0zx, iB_mu))).reshape(latent_dim, P, T, -1) #amélioré manon 
...
def predict(covar_module0, covar_module1, likelihood, train_xt, test_x, mu, z, P, T, id_covariate, eps):
    ...
    K0xz_iH_K0zx_iB_mu_st = torch.matmul(K0xz, torch.linalg.solve(H, torch.matmul(K0xz.T, iB_mu).unsqueeze(dim=1))).reshape(P, T, -1) #amélioré manon
    ...
    K0Xz_iK0zz_K0zx_mu_tilde = torch.matmul(K0Xz, torch.linalg.solve(K0zz, torch.matmul(K0xz.T, mu_tilde).unsqueeze(dim=1))).squeeze() #amélioré manon
