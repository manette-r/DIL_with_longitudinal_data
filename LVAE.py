...
if __name__ == "__main__":
    ...
    if run_validation:
        if dataset_type == 'HealthMNIST':
            validation_dataset = HealthMNISTDatasetConv(csv_file_data=csv_file_validation_data,
                                                        ...
                                                        transform=transforms.ToTensor(), bool_original=True, val_dataset_type='validation')
    ...
    try:
        ...
        nnet_model.load_state_dict(torch.load((gp_model_folder+'/final-vae_model.pth'), map_location=torch.device('cpu')))
    ...
    nnet_model.eval()
    iteration = 0
    print('setup_dataloader',len(setup_dataloader))
    with torch.no_grad():
    ...
    if hensman:
        ...
        try:
            gp_model.load_state_dict(torch.load((gp_model_folder+ '/gp_model.pth'), map_location=torch.device(device)))
            zt_list = torch.load((gp_model_folder+'/zt_list.pth'), map_location=torch.device(device))
        ...
        try:
            m = torch.load((gp_model_folder+'/m.pth'), map_location=torch.device(device)).detach()
            H = torch.load((gp_model_folder+'/H.pth'), map_location=torch.device(device)).detach()
        ...
        for i in range(0, latent_dim):
            ...
            try:
                gp_models[i].load_state_dict(torch.load((gp_model_folder+gp_model_name), map_location=torch.device(device)))
                zt_list[i] = torch.load((gp_model_folder+zt_list_name), map_location=torch.device('cpu'))
            except:
                pass
    ...
    if run_validation:
        ...
        with torch.no_grad():
            ...
            covar_module0.eval()
            covar_module1.eval()
            validate(nnet_model, type_nnet, validation_dataset, type_KL, num_samples, latent_dim, covar_module0, covar_module1, likelihoods, zt_list, T, weight, full_mu, prediction_x, id_covariate, loss_function, eps=1e-6, save_path=save_path)
